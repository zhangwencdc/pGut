...
while(1){
	...
	$l =~ s/[^0-9a-zA-Z]+$//;  
	my @a=split"\t",$l;
	if($a[1] eq "ProjectA" || $a[1] eq "China"){
	$sample{$a[0]}=$a[2];
	}
}
close F;
...
while(1){
	...
	my @target=keys %sample;
	foreach my $target (@target) {
		unless($target=~/[0-9a-zA-Z]/){next;}
		my $t=basename($target);
		$target="/home/zhangwen/project/2024Time/Data/CMP/".$sample{$target};
		my $out=$name."_".$t.".blat";
		print "blat $input $target $out\n";
