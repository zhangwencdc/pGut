...
# (1) 计算Bray-Curtis距离
# 读取质粒矩阵
plasmid_matrix <- read.delim("Plasmid_cluster.matrix", header=TRUE, row.names=1)
...
# 读取样本信息
sample_info <- read.delim("SampleID_factor.txt", header=TRUE)
samples <- rownames(sample_plasmid)
sample_info <- sample_info[match(samples, sample_info$ID), ]

# (2) 分组比较与小提琴图
group_vars <- c("Year","City","Gendor","Age","BMI","Smoke","Drink","Physical_experience","Yogurt","Fruit")
