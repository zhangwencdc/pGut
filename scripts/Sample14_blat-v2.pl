...
my $data="/home/zhangwen/project/2024Time/Analysis/HMP_Analysis/SingleCellseq/Sample14";

my @target=glob "$data/*.fasta";
open(OUT,">Sample14.plasmid.result");
...
while(1){
	...
	my $name=basename($input);my $total;my $match;my @match;
	foreach my $target (@target) {
		...
		if($size>0){
		if(($kn/$size)>=0.7){$match++;push @match,$t;}
		}
		close F;
	}
...
#	system "rm -rf *.blat\n";
	...
	print OUT "$line,$total,$match,@match\n";
