#Usage:OTU矩阵/Genus矩阵 与耐药基因矩阵 比较 寻找相关
if (!require("psych"))
...
option_list <- list(
  ...
  make_option(c("-m", "--meta"), type="character", help="Input file 2:colnames is SampleID [Required]")#Merge_Metaphlan_Genus.txt
...
ps4<-filter_taxa(ps2,function(x) sum (x>0)>=10,TRUE) #仅保留存在超过5个以上样本的Genus
...
write.csv(cor_df,"Psych_result_Plasmid_Genus.csv",quote = FALSE,row.names = FALSE)
